                            ...
                            Tesseract.recognize(
                                ...
                                .then(({ data, info }) => {
                                    Tesseract.recognize(
                                        ...
                                        str = text.replace(/(.|\s)+?(?=Måndag)|/m, "").replace(/\./g, ""); // Remove Weird characters that are not relevant
                                        days = str.split(/(Måndag|Tisdag|Onsdag|Torsdag|Fredag).*\s/ig).filter(n => n); // Split into each weekday and filter empty
